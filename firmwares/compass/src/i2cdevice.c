...
bool ICACHE_FLASH_ATTR i2cdevice_read(uint8_t devaddr, uint8_t regaddr, uint8_t len, uint8_t *data) {
	i2c_master_start();
...
bool ICACHE_FLASH_ATTR i2cdevice_write(uint8_t devaddr, uint8_t regaddr, uint8_t len, uint8_t *data) {
	//os_printf("Writing %x to %x\r\n", *data, regaddr);
	i2c_master_start();
...
bool ICACHE_FLASH_ATTR i2cdevice_writebit(uint8_t devaddr, uint8_t regaddr, uint8_t bit, bool val) {
	...
	if (val)
		...
		cbi(dat, bit);
	if (!i2cdevice_write(devaddr, regaddr, 1, &dat)) return false;
	return true;
}
