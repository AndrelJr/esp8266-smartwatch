...
/** Internal commands **/
void ICACHE_FLASH_ATTR ssd1306_putbyte(uint8_t byte) {
...
/** Graphics **/
void ICACHE_FLASH_ATTR ssd1306_setpixel(uint8_t x, uint8_t y, bool state) {
	if (state)
		sbi(buf[y / 8][x], y % 8);
	else
		cbi(buf[y / 8][x], y % 8);
...
void ICACHE_FLASH_ATTR ssd1306_line(uint8_t startx, uint8_t starty, uint8_t endx, uint8_t endy, bool state) {
	float x = startx;
	float y = starty;
	float max = abs(endx - startx) > abs(endy - starty) ? abs(endx - startx) : abs(endy - starty);

	while (((endx > startx) ? x < endx : x > endx) || ((endy > starty) ? y < endy : y > endy)) {
		x += (endx - startx) / max;
		y += (endy - starty) / max;
		ssd1306_setpixel((uint8_t)x, (uint8_t)y, state);
	}
}
